...
$volumes = @{
  ...
  "/workspace/mnt/repos"   = $testDirRepos
...
$env = @{
  ...
  "MANAGED_REPOS_CSV" = "/workspace/mnt/scripts/test-repos.csv"
}
...
if (Test-ContainerRunning -Name $containerName) {
  ...
  $repoUtils = Invoke-ContainerCommand -Container $containerName -Command "test -f /workspace/util/repo-utils.sh && echo 'exists'"
  Assert-Contains -Haystack $repoUtils -Needle "exists" -Message "repo-utils.sh exists"
    
  # Test 2: repo-utils.sh can be sourced
  $sourceTest = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/repo-utils.sh && echo 'sourced'"
  ...
  $ashrcCheck = Invoke-ContainerCommand -Container $containerName -Command "grep 'repo-utils.sh' /workspace/.ashrc"
  ...
  $csvCheck = Invoke-ContainerCommand -Container $containerName -Command "test -f /workspace/mnt/scripts/test-repos.csv && echo 'exists'"
  Assert-Contains -Haystack $csvCheck -Needle "exists" -Message "CSV file is mounted correctly"
    
  # Test 5: CSV file has correct format
  $csvContent = Invoke-ContainerCommand -Container $containerName -Command "cat /workspace/mnt/scripts/test-repos.csv"
  ...
  $listCmd = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/repo-utils.sh && type list_all_repos 2>&1"
  ...
  $fetchCmd = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/repo-utils.sh && type fetch_all_repos 2>&1"
  ...
  $showCmd = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/repo-utils.sh && type show_all_local_changes 2>&1"
  ...
  $statusCmd = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/repo-utils.sh && type repo_status 2>&1"
  ...
  Assert-Equal -Actual $envCheck.Trim() -Expected "/workspace/mnt/scripts/test-repos.csv" -Message "MANAGED_REPOS_CSV environment variable is set"
  ...
  $fetchOutput = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/repo-utils.sh && fetch_all_repos 2>&1"
  ...
  $repoExists = Invoke-ContainerCommand -Container $containerName -Command "test -d /workspace/mnt/repos/my-container-images/.git && echo 'cloned'"
    
  if ($repoExists -match "cloned") {
    Write-TestPass "Repository was cloned successfully"
        
    # Test 13: Verify it's a valid git repository
    $gitCheck = Invoke-ContainerCommand -Container $containerName -Command "cd /workspace/mnt/repos/my-container-images && git status 2>&1"
    ...
    $listOutput = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/repo-utils.sh && list_all_repos 2>&1"
    ...
    $changesOutput = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/repo-utils.sh && show_all_local_changes 2>&1"
    ...
    $makeChanges = Invoke-ContainerCommand -Container $containerName -Command "echo 'test change' >> /workspace/mnt/repos/my-container-images/test.txt"
    $changesOutput2 = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/repo-utils.sh && show_all_local_changes 2>&1"
    ...
    $repoStatusOutput = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/repo-utils.sh && repo_status my-container-images 2>&1"
  ...
  # Test 18: Example CSV file exists (in /workspace/util where scripts are copied)
  $exampleCsv = Invoke-ContainerCommand -Container $containerName -Command "test -f /workspace/util/managed-repos-example.csv && echo 'exists'"
