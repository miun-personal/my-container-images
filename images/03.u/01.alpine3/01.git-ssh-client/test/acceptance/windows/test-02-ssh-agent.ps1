...
if (Test-ContainerRunning -Name $containerName) {
    ...
    $sourceTest = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/agent-init.sh && echo 'sourced'"
    ...
    $agentPid = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/agent-init.sh && env | grep SSH_AGENT_PID"
    ...
    $socketCheck = Invoke-ContainerCommand -Container $containerName -Command ". /workspace/util/agent-init.sh && test -S `$SSH_AUTH_SOCK && echo 'exists'"
    ...
    $ashrcCheck = Invoke-ContainerCommand -Container $containerName -Command "grep 'agent-init.sh' /workspace/.ashrc"
...
if (Test-ContainerRunning -Name $containerName2) {
    ...
    $keygenScript = Invoke-ContainerCommand -Container $containerName2 -Command "test -x /workspace/util/setup-ssh-key.sh && echo 'executable'"
    ...
    $sshDir = Invoke-ContainerCommand -Container $containerName2 -Command "test -d /workspace/home/dev/.ssh && echo 'exists' || mkdir -p /workspace/home/dev/.ssh && echo 'created'"
    ...
    Write-TestInfo "Manual test: Run /workspace/util/setup-ssh-key.sh test@example.com inside container"
