...
The `git-ssh-overwatch-devcontainer` container image provides a minimal, secure, and reproducible Alpine Linux-based environment specifically designed for VS Code devcontainer workflows. This image is designed to support development workflows that require:

- Git operations with SSH key-based authentication
- Commit and tag signing using SSH keys (GPG.SSH format)
- Writable container filesystem for typical devcontainer usage
- User configuration that persists in the container (not mounted volumes)
- Automated SSH agent management for secure key handling

## Primary Use Cases

1. **VS Code Devcontainer**: Primary use as a VS Code devcontainer for Git-based projects
2. **Interactive Development**: Interactive development environment with writable home directory
...
- Developers using VS Code devcontainers for Git-based projects
- Developers requiring signed commits and secure SSH authentication in devcontainer workflows
- Organizations enforcing commit signing and sign-off policies (e.g., DCO compliance)
- Users requiring isolated Git environments with writable container filesystems
...
-|-|-
`/workspace/home/dev`|User home directory (SSH keys, Git config)|Yes (container filesystem)
...
### FR-8: Writable Container Filesystem

**The container filesystem must be writable for devcontainer workflows.**

- User home directory remains writable in the container filesystem
- No requirement for read-only root filesystem
- Temporary files use `/tmp` or `/dev/shm`
- Suitable for ephemeral container workflows
...
- Writable container filesystem (no read-only requirement)
- SSH keys stored with appropriate permissions (0600)
- SSH agent socket secured in `/dev/shm`
- No secrets embedded in image
- Credentials stored in container filesystem or provided via volume mounts
...
- No `read_only` flag requirement (writable filesystem)
- Support `user:` specification

### Host Integration

- SSH keys stored in container filesystem (not typically mounted from host)
- Git repositories from host can be mounted as volumes
- Custom scripts can be provided via volume mounts
- SSH agent runs in container (not forwarded from host)
...
2. **Security Best Practices**: Follow container security best practices (non-root, minimal attack surface)
...
2. It runs successfully with writable container filesystem (no read-only requirement)
...
9. Volume mounts work as documented for repositories
...
15. User home directory remains writable in container
