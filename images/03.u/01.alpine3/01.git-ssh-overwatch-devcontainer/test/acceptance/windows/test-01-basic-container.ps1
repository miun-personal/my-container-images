...
if (Test-ContainerRunning -Name $containerName) {
  ...
  Assert-Equal -Actual $userId.Trim() -Expected "1001" -Message "Container runs as non-root user (UID 1001)"
  ...
  $workspaceHome = Invoke-ContainerCommand -Container $containerName -Command "test -d /work && echo 'exists'"
  ...
  $utilDir = Invoke-ContainerCommand -Container $containerName -Command "test -d /work/util && echo 'exists'"
  ...
  $agentScript = Invoke-ContainerCommand -Container $containerName -Command "test -f /work/util/agent-init.sh && echo 'exists'"
  Assert-Contains -Haystack $agentScript -Needle "exists" -Message "agent-init.sh script exists"
    
  # Test 11: Check gitconfig.sh exists
  $gitconfigScript = Invoke-ContainerCommand -Container $containerName -Command "test -f /work/util/gitconfig.sh && echo 'exists'"
  ...
  $sshKeyScript = Invoke-ContainerCommand -Container $containerName -Command "test -f /work/util/setup-ssh-key.sh && echo 'exists'"
  ...
  $repoUtilsScript = Invoke-ContainerCommand -Container $containerName -Command "test -f /work/util/repo-utils.sh && echo 'exists'"
  ...
  $executableCheck = Invoke-ContainerCommand -Container $containerName -Command "test -x /work/util/agent-init.sh && echo 'executable'"
  ...
  Assert-Equal -Actual $envVar.Trim() -Expected "/work" -Message "WORKSPACE_HOME environment variable is set"
  ...
  Assert-Equal -Actual $userHome.Trim() -Expected "/work/home/dev" -Message "WORKSPACE_USER_HOME environment variable is set"
    
  # Test 17: Check .ashrc file exists
  $ashrc = Invoke-ContainerCommand -Container $containerName -Command "test -f /work/.ashrc && echo 'exists'"
  ...
  $aliases = Invoke-ContainerCommand -Container $containerName -Command "grep -c 'alias' /work/.ashrc"
  if ([int]$aliases -gt 0) {
...
if (Test-ContainerRunning -Name $containerName2) {
  ...
  $writeTest = Invoke-ContainerCommand -Container $containerName2 -Command "echo 'test' > /work/home/dev/test.txt && cat /work/home/dev/test.txt"
